...
AIClient = OpenAI(api_key=settings.openai_key)
if settings.AIAssistantID:
    AIAssistant = AIClient.beta.assistants.update(
        settings.AIAssistantID, instructions=open_file("system_mini.txt")
    )
else:
...
def update_db():
    ...
    cursor = con.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS threads (id INTEGER PRIMARY KEY, timestamp, thread_id TEXT, user_id TEXT)"
    )
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS subscribers (id INTEGER PRIMARY KEY, user_id TEXT, service TEXT)"
    )
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS chat (id INTEGER PRIMARY KEY, timestamp, role TEXT, user TEXT, chat TEXT, message TEXT, tokens INTEGER, costs REAL)"
    )
    con.commit()
    con.close()
...
def append_message(message, content, message_role="assistant"):
    ...
    thread = cursor.execute(
        "SELECT * FROM threads where user_id = ? ORDER BY timestamp desc limit 1",
    ...
    if not thread:
        ...
        cursor.execute(
            "INSERT INTO threads (timestamp, thread_id, user_id) VALUES (datetime('now'), ?, ?)",
            (ai_thread.id, message.from_user.id),
        )
    else:
    ...
    if ai_thread:
        AIClient.beta.threads.messages.create(
            ai_thread.id, role=message_role, content=content
        )
...
def get_response(message, content):
    ...
    subscription = cursor.execute(
        "select * from subscribers where user_id = ? and service = ?",
        (message.from_user.id, "AICHAT"),
    ).fetchall()
    if len(subscription) > 0:
        ...
        if ai_thread:
            send_message = AIClient.beta.threads.messages.create(
                ai_thread.id, role="user", content=content
            )
            print("Sent message:", content)

            message_run = AIClient.beta.threads.runs.create(
                thread_id=ai_thread.id, assistant_id=AIAssistant.id
            )

            while message_run.status != "completed":
                message_run = AIClient.beta.threads.runs.retrieve(
                    thread_id=ai_thread.id, run_id=message_run.id
                )
                print(message_run.status)

            messages = AIClient.beta.threads.messages.list(thread_id=ai_thread.id)
...
def reset(message):
    try:
        ...
        response = get_response(
            message, "Hello. My name is " + message.from_user.first_name
        )
...
def subscribe(message):
    try:
        ...
        if len(tokens) < 2:
            bot.reply_to(
                message, "Please specify the service you'd like to subscribe to"
            )
        else:
            ...
            subscription = cursor.execute(
                "select * from subscribers where user_id = ? and service = ?",
                (message.from_user.id, service),
            ).fetchall()
            if len(subscription) > 0:
                bot.reply_to(
                    message,
                    "You are already subscribed to " + service + " subscription",
                )
            else:
                failed = False
                if service == "AICHAT":
                    if len(tokens) < 3:
                        bot.reply_to(
                            message, "Your subscription to " + service + " failed"
                        )
                        failed = True
                    else:
                ...
                if not failed:
                    cursor.execute(
                        "INSERT INTO subscribers (user_id, service) VALUES (?, ?)",
                        (message.from_user.id, service),
                    )
                    con.commit()
                    con.close()
                    bot.reply_to(
                        message,
                        "You have succesfully subscribed to "
                        + service
                        + " subscription",
                    )
    except Exception as e:
...
def unsubscribe(message):
    try:
        ...
        if len(tokens) < 2:
            bot.reply_to(
                message, "Please specify the service you'd like to unsubscribe to"
            )
        else:
            service = tokens[1].upper()
            con = sqlite3.connect(os.path.join(script_dir, "chatter.db"))
            ...
            if len(subscription) > 0:
                cursor.execute(
                    "delete from subscribers where user_id = ? and service = ?",
                    (message.from_user.id, service),
                )
                con.commit()
                con.close()
                bot.reply_to(
                    message,
                    "You have been unsubscribed from " + service + " subscription",
                )
            else:
                bot.reply_to(
                    message, "You are not subscribed to " + service + " subscription"
                )
    except Exception as e:
        bot.reply_to(message, "Sorry, " + str(e))
...
def msg_length(message):
    try:
        ...
        messages = cursor.execute(
            "SELECT count(*) as msglength,sum(tokens) as tokensum, sum(costs) as cost_total FROM chat WHERE chat = ?",
            (message.chat.id,),
        ).fetchall()
        response = (
            "Message Id:"
...
def stock(message):
    try:
        ...
        response = get_response(
            message, "Ticker " + ticker + " candles: " + str(candles)
        )
        bot.reply_to(message, response)
    except Exception as e:
        bot.reply_to(message, "Sorry, " + str(e))
    ...
    try:
        ...
        if len(tokens) > 1:
            for tk in tokens[1:]:
                ...
                if fresult.empty:
                    bot.reply_to(
                        message, "Info for ticker " + tk + " could not be found"
                    )
                else:
...
def stock_levels(message):
    ...
    minute_candles = yqticker.history(
        start=minute_start_date, end=end_date, interval="5m"
    )
    ...
    for cidx in range(datalen):
        ...
        if resistancelevels[curcluster] > 2:
            if curcluster not in donecluster:
                ...
                finalreslevels[curcluster] = {
                    "level": (finalreslevels[curcluster]["level"] + candle["high"]) / 2,
                    "count": finalreslevels[curcluster]["count"] + 1,
                }
    ...
    for cidx in range(datalen):
        ...
        if supportlevels[curcluster] > 2:
            if curcluster not in donecluster:
                ...
                finalsuplevels[curcluster] = {
                    "level": (finalsuplevels[curcluster]["level"] + candle["low"]) / 2,
                    "count": finalsuplevels[curcluster]["count"] + 1,
                }
                        ...
                        "content": [
                            {"type": "text", "text": message.caption},
                            {
                                ...
                                "image_url": {
                                    ...
                                    + encode_image(
                                        os.path.join(script_dir, file_info.file_path)
                                    ),
                                },
                            },
                        ],
...
def voice_processing(message):
    ...
    if len(subscription) > 0:
        try:
            ...
            with open(
                os.path.join(script_dir, "voices", filename + ".ogg"), "wb"
            ) as new_file:
                new_file.write(downloaded_file)
            ogg_audio = AudioSegment.from_file(
                os.path.join(script_dir, "voices", filename + ".ogg"), format="ogg"
            )
            ogg_audio.export(
                os.path.join(script_dir, "voices", filename + ".mp3"), format="mp3"
            )
            transcript = AIClient.audio.transcriptions.create(
                model="whisper-1",
                file=open(os.path.join(script_dir, "voices", filename + ".mp3"), "rb"),
            )
        ...
        try:
            ...
            if "Response image:" in response:
                try:
                    ...
                    response_image = AIClient.images.generate(
                        model="dall-e-3", prompt=resp_prompt[1], n=1, size="1024x1024"
                    )
